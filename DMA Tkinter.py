...
def run_program():
        ...
        while current_voltage <= voltage_end:
            ...
            while repeat_readings < dwell_steps:
                ...
                electrometer_conc.append(
                    electrometer_voltage[-1] * 6.242e6 * 60 / flow_rate
                )
            ...
            time_from_start_avg.append(
                sum(time_from_start[-dwell_steps:]) / dwell_steps
            )
            dma_voltage_avg.append(sum(dma_voltage[-dwell_steps:]) / dwell_steps)
            electrometer_conc_avg.append(
                sum(electrometer_conc[-dwell_steps:]) / dwell_steps
            )
    ...
    with open(filename_avg, "w", newline="") as csvfile:
        ...
        # Write CSV file header header
        data_writer.writerow(
            ["Time Since Start", "DMA Voltage", "Electrometer Concentration"]
        )
        for x in range(len(time_from_start_avg)):
            data_writer.writerow(
                [time_from_start_avg[x], dma_voltage_avg[x], electrometer_conc_avg[x]]
            )
...
BertanStop = ttk.Entry(BertanFrame, textvariable=voltage_stop, width=13).grid(
    row=3, column=5, padx=10
)  # change e1 to BertanStop
...
BertanVoltSet = Button(
    BertanFrame, text="Start", width=5, bg="springgreen", command=run_program
